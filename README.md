...
Three interchangeable `LLMClient` implementations (`app/services/llm_client.py`), selected by `LLM_PROVIDER`:

- **`AnthropicLLMClient`** (`LLM_PROVIDER=anthropic`) — real Claude tool-calls for extraction, intent, and summarisation, with every call wrapped so a timeout or malformed response falls back to the rule-based client rather than breaking the call.
- **`SarvamLLMClient`** (`LLM_PROVIDER=sarvam`) — real calls to Sarvam AI's OpenAI-compatible chat-completions endpoint (`LLM_BASE_URL`, default `https://api.sarvam.ai/v1`), prompted for strict-JSON responses matching the same extraction/intent schemas Anthropic uses. **Unverified against a live Sarvam key** — same status Vapi was in before it got tested against a real account (§16): built against Sarvam's documented API shape, every call wrapped so a wrong assumption about the exact envelope (auth header, endpoint path, JSON-mode support) only means a silent fallback to the rule-based client, never a broken call. Confirm against a live key and fix in `SarvamLLMClient` only — nothing else should need to change.
- **`RuleBasedLLMClient`** — a zero-dependency fallback (regex/keyword-driven) used automatically whenever no `LLM_API_KEY` is configured, or if the configured provider's call fails. **This is what the entire test suite and the adversarial testing below run against** — the core orchestration logic was built and hardened against this before any live LLM call was ever needed, per the brief's own rule: "If an external dependency isn't available, create a clean mock interface so development can continue."
...
**Current state (as of this build): `VOICE_PROVIDER=mock`.** A Vapi private
API key and a Custom-LLM assistant (`VAPI_ASSISTANT_ID`) are already wired
into `.env` and verified against the live Vapi API — but the Vapi account
has **zero phone numbers provisioned**, so no real call can be placed yet.
**To go live: buy/import a phone number in the Vapi dashboard (this costs
real money — Vapi bills per-minute call credits, buying a number has its
own cost) and add its ID as `VAPI_PHONE_NUMBER_ID`**, run a tunnel
(ngrok/cloudflared) so Vapi's webhook can reach this backend, point
`PUBLIC_BASE_URL` (and the assistant's `model.url`/`server.url`, both via
the Vapi API) at that tunnel URL, then flip `VOICE_PROVIDER=vapi`. No other
code changes are needed — the provider, webhook adapter, and assistant are
already built and wired; the remaining gap is purely "buy credits and get a
tunnel running," not engineering.
...
| `LLM_PROVIDER`, `LLM_MODEL`, `LLM_API_KEY` | LLM configuration — `anthropic` or `sarvam`; no key is hardcoded anywhere; empty key = rule-based fallback |
| `LLM_BASE_URL` | Only used when `LLM_PROVIDER=sarvam` — Sarvam's API base URL |
